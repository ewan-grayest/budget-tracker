...
- `PORT` — порт внутри контейнера, по умолчанию `8080` (`HEALTHCHECK` берёт его
  из той же переменной, поэтому переопределение не ломает проверку);
- `HOST` — интерфейс, который слушает сервер, по умолчанию `0.0.0.0`;
...
`docker-compose.yml` запускает сервис с ограничениями, которые стоит учитывать
при изменении конфигурации:

- корневая файловая система смонтирована только для чтения (`read_only: true`);
  записывать можно в том `/data` и в `tmpfs` на `/tmp` (нужен SQLite для
  временных файлов). Если приложению понадобится писать куда-то ещё, добавьте
  соответствующий том;
- сняты все capabilities (`cap_drop: ALL`) и запрещено повышение привилегий
  (`no-new-privileges`); процесс и так работает от непривилегированного `appuser`;
- лимиты `mem_limit: 256m` и `cpus: 1.0`;
- логи json-file ротируются (`max-size: 10m`, `max-file: 3`) — приложение пишет
  в stdout строку на каждый запрос;
- `init: true` — форвардинг сигналов и сбор осиротевших процессов. Приложение
  дополнительно само обрабатывает `SIGTERM` и завершается штатно, поэтому
  `docker stop` не ждёт таймаут и не обрывает запись в SQLite.
