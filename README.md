...
непривилегированный процесс писать в него не может. Точка входа умеет это
чинить, но только если контейнер стартовал от root: она создаёт `DATA_DIR`,
передаёт его `RUN_UID:RUN_GID` и лишь затем сбрасывает привилегии и запускает
приложение — под root оно не работает никогда.

```bash
# том принадлежит root: стартуем от root, точка входа сбросит привилегии сама
docker run -d -v budget_data:/data --user 0:0 \
  --cap-add CHOWN --cap-add FOWNER --cap-add DAC_OVERRIDE \
  --cap-add SETUID --cap-add SETGID \
  -p 8080:8080 budget-manager:local
```

В `docker-compose.yml` для этого есть `CONTAINER_USER=0:0` и закомментированный
блок `cap_add`. Без capabilities `chown` не выполнится: точка входа сообщит об
этом и продолжит, а приложение остановится с понятным сообщением, если каталог
всё же окажется недоступным на запись.

Если платформа умеет выставлять владельца тома сама (`fsGroup` в Kubernetes,
`--user` с нужным UID), root не нужен — оставьте конфигурацию по умолчанию.
